        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="heading-elements">
                    @if ($data['dataModel']->status != 'COMPLETED' && $data['dataModel']->status != 'CANCEL')
                    <a href="{{ route('admin.order.destroy', $data['dataModel']->id) }}" data-id="{{ $data['dataModel']->id }}" class="btnCancel">
                        <button type="button" class="btn btn-danger btn-xs heading-btn pull-right"><i class="icon-cross position-left"></i> Cancel</button>
                    </a>
                    @endif

                    @if ($data['dataModel']->status == 'CONFIRM')
                    <a href="{{ route('admin.order.status', $data['dataModel']->id) }}" data-id="{{ $data['dataModel']->id }}" data-status="PAID" class="btnStatus">
                        <button type="button" class="btn btn-success btn-xs heading-btn pull-right"><i class="icon-file-check position-left"></i> PAID </button>
                    </a>
                    @endif

                    @if ($data['dataModel']->status == 'PAID')
                    <a href="{{ route('admin.order.status', $data['dataModel']->id) }}" data-id="{{ $data['dataModel']->id }}" data-status="COMPLETED" class="btnStatus">
                        <button type="button" class="btn btn-success btn-xs heading-btn pull-right"><i class="icon-file-check position-left"></i> COMPLETED</button>
                    </a>
                    @endif
                    <a href="{{ route('admin.order.print', $data['dataModel']->id) }}" target="_blank" class="btnPrint">
                        <button type="button" class="btn btn-default btn-xs heading-btn pull-right"><i class="icon-printer position-left"></i> Print</button>
                    </a>
                </div>
            </div>

            <div class="card-body no-padding-bottom" id="printJS-form">
                <div class="row">
            ...
            swal({
                ...
                if(result.value) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(){
                            blockMessage($('#tableData'),'Please Wait , {{ "Processing data" }}','#fff');
                        }
                        }).done(function(data){
                            $('#tableData').unblock();
                            if(data.Code == 200){
                    ...
                    showNotif("default","Message","Canceled");
                }
            });
        });
